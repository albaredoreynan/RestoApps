...
row5 << [@sale.branch.concept.name, @sale.branch.name, @sale.created_at.strftime("%-d-%b-%Y") ]
...
row1 << ['Sales By Category','','']
  ...
  @sale.sale_sale_categories.each do |scr|
    ...
    row1 << [scr.sale_category.name, number_to_currency(scr.amount, :unit => peso_sign), number_to_percentage(amount_percentage, :precision => 2)]
  end
